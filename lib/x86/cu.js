...
proto._opcodeTable = {
  ...
  , 0xb6: [ '_movi'     , [ 'mov'  , 'bh' ]  , BYTE ]
  , 0xb7: [ '_movi'     , [ 'mov'  , 'dh' ]  , BYTE ]
...
proto.__movi_b = function (regName, value) {
  var fullName, lower;
  fullName = this.regs.byte_l[regName]

  if (fullName) lower = true;
  else fullName = this.regs.byte_h[regName];

  var reg = this.regs[fullName];
  var bytes = [
      reg & 0xff000000  // upper byte of upper word
    , reg & 0x00ff0000  // lower byte of upper word
    , reg & 0x0000ff00  // ah
    , reg & 0x000000ff  // al
  ]
  ...
  if (lower) {
    this.regs[fullName] = bytes[0] | bytes[1] | bytes[2] | value
  } else {
    this.regs[fullName] = bytes[0] | bytes[1] | (value << 8) | bytes[3]
  }
}
...
proto._movi = function _movi(opcode, asm, nbytes) {
  ...
  if (nbytes === BYTE) this.__movi_b(regName, imd)
  else regs[regName] = imd;
