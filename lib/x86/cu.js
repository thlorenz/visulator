...
proto.next = function next() {
  ...
  // in case the size of the instruction was overridden
  // we also adjust the target register
  // (second part of assembly instruction)
  if (this._adaptRegName) {
    asm[1] = this._adaptRegName(asm[1])
    this._adaptRegName = null
  }
...
proto._opcodeTable = {
  ...
  , 0x66: [ '_opsizeOverride' , [ 'no'   , 'mnemonic' ] , BYTE  ]
  , 0x89: [ '_movr'           , [ 'r1/32', 'r2/32'    ] , DWORD ]
 ...
 * mov, add, etc.
 *
 * Same code used no matter of the pair size.
 *
...
proto._movr = function _movr(opcode, asm, nbytes) {
  ...
  // next byte in code tells us which reg to move into which
  // the different permutations are just numbered through
  // consecutively from c0-ff so we use a table
  var regPairCode = this.mem.load(regs.eip, BYTE);
  var regPair = this._regPair[regPairCode];
...
proto._opsizeOverride = function _opsizeOverride(opcode, asm, size) {
  this._sizeOverride = WORD;
  this._adaptRegName = getWordRegName;
