...
proto.next = function next() {
  ...
  debug('addr: %s, opcode: %s', hex(codeAddr), opcodeString)
...
proto._inc = function _inc(opcode, asm, dstbytes, srcbytes) {
  this.__inc_dec(opcode, asm, dstbytes, false)
}
...
proto._dec = function _dec(opcode, asm, dstbytes, srcbytes) {
  this.__inc_dec(opcode, asm, dstbytes, true)
}

proto.__inc_dec = function _inc_dec(opcode, asm, dstbytes, sub) {
  var regs = this.regs
    , regName
  ...
  var res = sub
    ? this.alu.dec(dst, dstbytes)
    : this.alu.inc(dst, dstbytes);
...
proto._inc_dec_b = function _inc_dec(opcode, asm, dstbytes, srcbytes) {
  assert(dstbytes === BYTE, 'this only handles BYTE sized inc/dec ops')
  var regs = this.regs
...
proto._xori_al = function _xori_al(opcode, asm, dstbytes, srcbytes) {
  this.__logical_opi_eaxl(dstbytes, srcbytes, instruction.xor)
}

proto._xori_eax = function _xori_eax(opcode, asm, dstbytes, srcbytes) {
  this.__logical_opi_eaxl(dstbytes, srcbytes, instruction.xor)
}

proto._xorr = function _xorr(opcode, asm, dstbytes, srcbytes) {
  this.__logical_opr(dstbytes, srcbytes, instruction.xor)
}

// 'test' is the same as 'and' except it doesnt affect operands
proto._testr = function _testr(opcode, asm, dstbytes, srcbytes) {
  this.__logical_opr(dstbytes, srcbytes, instruction.and, true)
}

proto._testi_al = function _testi_al(opcode, asm, dstbytes, srcbytes) {
  this.__logical_opi_eaxl(dstbytes, srcbytes, instruction.and, true)
}

proto._testi_eax = function _testi_eax(opcode, asm, dstbytes, srcbytes) {
  this.__logical_opi_eaxl(dstbytes, srcbytes, instruction.and, true)
}

proto.__logical_opr = function __logical_opr(dstbytes, srcbytes, op, nostore) {
  var regs = this.regs;
  ...
  if (!nostore) regs[regPair[0]] = res
  regs.eip += BYTE;
}

proto.__logical_opi_eaxl = function __logical_opi_eaxl(dstbytes, srcbytes, op, nostore) {
  // 0x0c: `or  al, ..`
  // 0x0d: `or  eax, ..`
  // 0x24: `and al, ..`
  var regs    = this.regs
...
proto._logical_opi_wdw = function _logical_opi_wdw(opcode, asm, dstbytes, srcbytes) {
  var regs    = this.regs
    , regCode = this.mem.load(regs.eip, BYTE)
  ...
  switch (inst.name) {
    ...
    case 'xor':
      // special case:
      // 66 83 f1 ff xor cx,0xffff
      if (srcbytes < dstbytes && src >= 0x88)
        src = fill(dstbytes, srcbytes, src)
      res = this.alu.xor(dst, src, dstbytes)
      regs[regName] = res
      break;
  }
...
proto.__test = function __test(dst, dstbytes, srcbytes) {
  var regs = this.regs;
  var src = leVal(this.mem.load(regs.eip, srcbytes))
  regs.eip += srcbytes

  var res = this.alu.and(dst, src, dstbytes)
  this._updateMetaAndFlags({
      src    : src
    , dst    : dst
    , bytes  : dstbytes
...
proto._not_neg = function _not_neg(opcode, asm, dstbytes, srcbytes) {
  var regs = this.regs;

  var regPairCode = this.mem.load(regs.eip, BYTE)
  regs.eip += BYTE;

  var regPair = cu_regPair.getRegPair(regPairCode, dstbytes)

  var dst = regs[regPair[0]]
    , op = cu_regPair.getInstructionFromRegCode(regPairCode, opcode)

  if (op === instruction.test) return this.__test(dst, dstbytes, srcbytes)

  var res = this.alu[op.name](dst, dstbytes)
...
proto._movi = function _movi(opcode, asm, dstbytes, srcbytes) {
  ...
  if (dstbytes === WORD) regName = cu_regName.getWordRegName(regName);

  var imdBytes = this.mem.load(regs.eip, dstbytes);
  var imd = leVal(imdBytes, dstbytes);
...
function localJump(eip, distance) {
  // forward
  if (distance < 0x80) return eip + distance;
  // backward since distance is negative
...
proto._jmp = function _jmp(opcode, asm, dstbytes, srcbytes) {
  ...
  res = localJump(regs.eip, distance)
...
proto.__conditionalJump = function __conditionalJump(opcode, asm, dstbytes, srcbytes, conditionMet) {
  var regs = this.regs
    , res;
  ...
  debug({
    asm      : asm,
    dstbytes : hex(dstbytes),
    srcbytes : hex(srcbytes),
    eip      : hex(regs.eip),
  ...
  if (conditionMet()) regs.eip = res
}

proto._jo = function _jo(opcode, asm, dstbytes, srcbytes) {
  var regs = this.regs
  function overflow() { return regs.isFlagSet('OF') }
  this.__conditionalJump(opcode, asm, dstbytes, srcbytes, overflow)
}

proto._jno = function _jno(opcode, asm, dstbytes, srcbytes) {
  var regs = this.regs
  function no_overflow() { return !regs.isFlagSet('OF') }
  this.__conditionalJump(opcode, asm, dstbytes, srcbytes, no_overflow)
}

proto._je = function _je(opcode, asm, dstbytes, srcbytes) {
  var regs = this.regs
  function zero() { return regs.isFlagSet('ZF') }
  this.__conditionalJump(opcode, asm, dstbytes, srcbytes, zero)
}

proto._jne = function _jne(opcode, asm, dstbytes, srcbytes) {
  var regs = this.regs
  function no_zero() { return !regs.isFlagSet('ZF') }
  this.__conditionalJump(opcode, asm, dstbytes, srcbytes, no_zero)
}
...
proto._adjustFlags = function _adjustFlags(opts) {
  ...
  if (opts.op === instruction.not) return
  ...
  var ZF = res === 0
    ...
    //  ensure we substract the dst reg from 0x0
    //  when determining AF for not/neg
    , AF = (opts.op === instruction.neg || opts.op === instruction.not)
        ? auxiliary(opts.src, opts.dst, opts.op, opts.sub)
        : auxiliary(opts.dst, opts.src, opts.op, opts.sub)

  debug({
      bytes   : opts.bytes
    , dst     : hex(opts.dst)
    , src     : opts.src !== hex(opts.src)
    , res     : hex(opts.res)
    , sub     : opts.sub
    , noModCF : opts.noModCF
  })
