...
proto._opcodeTable = {
  ...
  , 0x24: [ '_andi_al'        , [ 'and'      , 'al'       ]      , BYTE  , BYTE  ]
...
function getInstructionFromRegCode(regCode) {
...
proto._addsubiw_dw = function _addsubiw_dw(opcode, asm, dstbytes, srcbytes) {
  ...
  var inst = getInstructionFromRegCode(regCode)
  ...
  regName = dstbytes === DWORD ? getDWordRegName(regName)
          : dstbytes === WORD  ? getWordRegName(regName)
          : regName
  ...
  this._adjustFlags({
    ...
    , src    : src
    , res    : res
    , sub    : inst.name === 'sub' || inst.name === 'cmp'
  })
}
...
proto._adjustFlags = function _adjustFlags(opts) {
  var res = opts.res;
  var ZF = res === 0
    , OF = !opts.sub && overflow(opts.src, opts.dst, opts.res, opts.nbytes)
