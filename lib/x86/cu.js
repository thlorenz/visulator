...
proto.next = function next() {
  ...
  // _opsizeOverride may have forced the size of bytes used
  // for the instruction
  if (this._sizeOverride) {
    nbytes = overrideSizeTable[opcode] || WORD 
...
proto._opcodeTable = {
    0x00: [  ] // not used
  , 0x05: [ '_add_eax'        , [ 'add'      , 'eax'      ] , DWORD ]
  , 0x40: [ '_inc'            , [ 'inc'      , 'eax'      ] , DWORD ]
  , 0x41: [ '_inc'            , [ 'inc'      , 'ecx'      ] , DWORD ]
  , 0x42: [ '_inc'            , [ 'inc'      , 'edx'      ] , DWORD ]
  , 0x43: [ '_inc'            , [ 'inc'      , 'ebx'      ] , DWORD ]
  , 0x44: [ '_inc'            , [ 'inc'      , 'esp'      ] , DWORD ]
  , 0x45: [ '_inc'            , [ 'inc'      , 'ebp'      ] , DWORD ]
  , 0x46: [ '_inc'            , [ 'inc'      , 'esi'      ] , DWORD ]
  , 0x47: [ '_inc'            , [ 'inc'      , 'edi'      ] , DWORD ]
  , 0x48: [ '_dec'            , [ 'dec'      , 'eax'      ] , DWORD ]
  , 0x49: [ '_dec'            , [ 'dec'      , 'ecx'      ] , DWORD ]
  , 0x4a: [ '_dec'            , [ 'dec'      , 'edx'      ] , DWORD ]
  , 0x4b: [ '_dec'            , [ 'dec'      , 'ebx'      ] , DWORD ]
  , 0x4c: [ '_dec'            , [ 'dec'      , 'esp'      ] , DWORD ]
  , 0x4d: [ '_dec'            , [ 'dec'      , 'ebp'      ] , DWORD ]
  , 0x4e: [ '_dec'            , [ 'dec'      , 'esi'      ] , DWORD ]
  , 0x4f: [ '_dec'            , [ 'dec'      , 'edi'      ] , DWORD ]
  ...
  , 0x66: [ '_opsizeOverride' , [ 'for' , 'addi/movi/r'   ] , WORD  ]
  , 0x81: [ '_addidw'         , [ 'r/32'     , 'i/32'     ] , DWORD ]
  , 0x83: [ '_addiw'          , [ 'r/32/16'  , 'i/8'      ] , BYTE  ]
  ...
  , 0xfe: [ '_inc_dec_b'      , [ 'inc/dec'  , 'r/8'      ] , BYTE ]
}

// this table exists for the few instructions for which the
// size override is not WORD
var overrideSizeTable = {
  0x83: BYTE
}
 ...
 * @param nbytes
 */
proto._inc = function _inc(opcode, asm, nbytes) {
  this.__inc_dec(opcode, asm, nbytes, false)
}
 ...
 * @param nbytes
 */
proto._dec = function _dec(opcode, asm, nbytes) {
  this.__inc_dec(opcode, asm, nbytes, true)
}

proto.__inc_dec = function _inc_dec(opcode, asm, nbytes, sub) {
  var regs = this.regs
    , regName
  ...
  var res = sub ? this.alu.dec(dst, nbytes) : this.alu.inc(dst, nbytes);
  regs[regName] = res;
...
proto._inc_dec_b = function _inc_dec(opcode, asm, nbytes) {
  assert(nbytes === BYTE, 'this only handles BYTE sized inc/dec ops')
  var regs = this.regs
...
proto._add_eax = function _add_eax(opcode, asm, nbytes) {
  var src = leVal(this.mem.load(this.regs.eip, nbytes))
    , dst = this.regs.eax
    , res = this.alu.add(dst, src, nbytes)
    , sub = false
  ...
  this.regs.eip += nbytes;
}

proto._addiw =
proto._addidw = function _addidw(opcode, asm, nbytes) {
  var regs    = this.regs
    , regCode = this.mem.load(regs.eip, BYTE)
    , sub     = regCode >= 0xc8
  regs.eip += BYTE
  ...
  var regName = nbytes === BYTE && regCode < 0xc4
      ? this._byteRegPair[regCode][0]
      : this._regPair[regCode][0]

  // this version of addi always assigns to the dword register
  regName = getDWordRegName(regName)

  var dst = regs[regName]
    , src = leVal(this.mem.load(regs.eip, nbytes))
  regs.eip += nbytes

  debug({ regName: regName })
  // no matter what the size of src, DWORD of dst is considered
  // when determining overflow
  var res = this.alu.add(dst, src, DWORD)
  regs[regName] = res
