...
proto._opcodeTable = {
  // code:  method            , operation    , destination  , dst size, src size, override affects dst ]
    0x00: [  ] // not used
  , 0x05: [ '_add_eax'        , [ 'add'      , 'eax'      ] , null, DWORD ]
  ...
  , 0x81: [ '_addiw_dw'       , [ 'r/32'     , 'i/32'     ] , DWORD, DWORD , true ]
  , 0x83: [ '_addiw_dw'       , [ 'r/32/16'  , 'i/8'      ] , DWORD, BYTE , true ]
...
proto.__inc_dec = function _inc_dec(opcode, asm, srcbytes, sub) {
  ...
  var res = sub ? this.alu.dec(dst, srcbytes) : this.alu.inc(dst, srcbytes);
  regs[regName] = res;
...
proto._add_eax = function _add_eax(opcode, asm, dstbytes, srcbytes) {
  var src = leVal(this.mem.load(this.regs.eip, srcbytes))
    ...
    , res = this.alu.add(dst, src, srcbytes)
    , sub = false
...
proto._addiw_dw = function _addiw_dw(opcode, asm, dstbytes, srcbytes) {
  var regs    = this.regs
    , regCode = this.mem.load(regs.eip, BYTE)
    , sub     = regCode >= 0xc8
  ...
  if (sub) return this._subiw_dw(opcode, asm, dstbytes, srcbytes, regCode);
  ...
  var res = this.alu.add(dst, src, dstbytes)
  regs[regName] = res
...
proto._subiw_dw = function _subiw_dw(opcode, asm, dstbytes, srcbytes, regcode) {
  var regs = this.regs;
  regs.eip += BYTE

  // TODO: implement similarly to addiw_dw
}
