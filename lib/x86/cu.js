...
proto._opcodeTable = {
  ...
  , 0xfe: [ '_inc_dec'        , [ 'inc/dec'  , 'r/8'      ] , BYTE ]
}
...
proto._dec = function _dec(opcode, asm, nbytes) {
  ...
  switch(nbytes) {
    case WORD:
      ...
      break
    case BYTE:
      throw new Error('BYTE inc/dec ops are handled by _inc_dec')
      break
    default:
      regName = asm[1]
  }
...
proto._inc_dec = function _inc_dec(opcode, asm, nbytes) {
...
proto._adjustFlags = function _adjustFlags(opts) {
  var res = opts.res;
  debug({nbytes: opts.nbytes, dst: hex(opts.dst), src: hex(opts.src), res: hex(opts.res), sub: opts.sub, noModCF: opts.noModCF })
  if (res === 0) this.regs.setFlag('ZF'); else this.regs.clearFlag('ZF');
  if (!parity(res)) this.regs.setFlag('PF'); else this.regs.clearFlag('PF');
  if (signed(res, opts.nbytes)) this.regs.setFlag('SF'); else this.regs.clearFlag('SF');
